...
const SecurityScoreGauge: React.FC<{ score: number }> = ({ score }) => {
    ...
    const getColor = (s: number) => {
        ...
        if (s >= 60) return 'text-brand-yellow';
        return 'text-error';
    };
    ...
    return (
        <div className="relative w-24 h-24 flex items-center justify-center flex-shrink-0">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                ...
                <span className="text-[9px] uppercase font-bold text-text-secondary tracking-widest">{t('security_score')}</span>
            </div>
        </div>
    );
};
...
const SecurityScreen: React.FC = () => {
    const { t } = useLanguage();
    const { isBiometricEnabled, setupBiometrics } = useAuth();
    ...
    const hasSecurityQuestions = securitySettings?.hasSecurityQuestions ?? false;
    const biometricEnabled = securitySettings?.biometricEnabled ?? isBiometricEnabled;
    ...
    return (
        <PageLayout title={t('security')}>
            <div className="flex flex-col gap-8 pb-32 px-4 pt-6">
                ...
                <div className="bg-background-secondary p-6 rounded-3xl border border-border-divider flex flex-row items-center gap-6 shadow-lg relative overflow-hidden">
                    <div className={`absolute top-0 right-0 w-32 h-32 bg-${score >= 80 ? 'success' : 'brand-yellow'}/5 rounded-full blur-3xl -mr-10 -mt-10 pointer-events-none`}></div>
                    
                    <SecurityScoreGauge score={score} />
                    
                    <div className="z-10 min-w-0 flex-1">
                        <div className="flex items-center gap-2 mb-2 flex-wrap">
                            <RecommendationIcon className={`w-5 h-5 ${statusColor} flex-shrink-0`} />
                            <h2 className="text-lg font-bold text-text-primary whitespace-nowrap">{t('status')}: <span className={statusColor}>{statusText}</span></h2>
                        </div>
