...
const UsersPage: React.FC = () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        error('Load Failed', response.error || 'Failed to fetch users');
        setUsers([]);
        setTotalPages(1);
      }
    } catch (err) {
  ...
  const getKycBadge = (status: string) => {
    switch (status) {
      ...
      case 'NOT_VERIFIED': return <span className="badge badge-info">None</span>;
              ...
              {isLoading ? (
                ...
                users.map((user) => (
                  <tr key={user.id} className="hover:bg-background-tertiary/50 transition-colors">
                    ...
                    <td className="text-text-secondary">{new Date(user.createdAt).toLocaleDateString()}</td>
                    <td>
                          ...
                          <div className="absolute right-0 top-full mt-1 bg-background-tertiary border border-border-divider rounded-lg shadow-xl z-10 py-1 min-w-40">
                            ...
                            {user.isActive && !user.isBanned ? (
                              <button
                                ...
                                className="w-full px-4 py-2 text-left text-sm text-status-error hover:bg-background-secondary flex items-center gap-2"
                              >
                              ...
                              <button
                                ...
                                className="w-full px-4 py-2 text-left text-sm text-status-success hover:bg-background-secondary flex items-center gap-2"
                              >
